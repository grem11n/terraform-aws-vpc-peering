locals {
  ...
  # `this_dest_cidrs` represent CIDRs of peer VPC, therefore a destination CIDRs for this_vpc
  # `peer_dest_cidrs` represent CIDRs of this VPC, therefore a destination CIDRs for peer_vpc
  # Destination cidrs for this are in peer and vice versa
  # We iterate over all cidr_block_associations in case of additional CIDR blocks of VPC (aws_vpc.cidr_block returns only main block)
  this_dest_cidrs = length(var.peer_subnets_ids) == 0 ? toset([for s in data.aws_vpc.peer_vpc.cidr_block_associations : s.cidr_block]) : toset(data.aws_subnet.peer[*].cidr_block)
  peer_dest_cidrs = length(var.this_subnets_ids) == 0 ? toset([for s in data.aws_vpc.this_vpc.cidr_block_associations : s.cidr_block]) : toset(data.aws_subnet.this[*].cidr_block)
